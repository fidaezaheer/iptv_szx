                            ...
                            <div class="block-content collapse in">
                                <div class="span12">
                                   <div class="table-toolbar">
                                      <div class="btn-group">
                                         <a href="proxy_add.php?level=<?php echo $level ?>"><button class="btn btn-success"><?php echo $lang_array["proxy_list_text3"] ?><i class="icon-plus icon-white"></i></button></a>
                                      </div>
                                   </div>
...
<?php
			...
			$namess = array();
			if($level == 1)	
				$namess = $sql->fetch_datas_where_or_null($mydb, $mytable, "proxylevel", $level);
			else
				$namess = $sql->fetch_datas_where($mydb, $mytable, "proxylevel", $level);
		...
		function delete_user(name)
		{
			if(confirm("<?php echo $lang_array["proxy_list_text20"] ?>： " + name + " ?") == true)
  			{
				var url = "proxy_del.php?name=" + base64encode(encodeURI(name)) + "&level=<?php echo $level ?>";
		...
		function edit_user(name)
		{
			//if(confirm("是否删除用户： " + name + " ?") == true)
  			{
				var url = "proxy_edit.php?name=" + name + "&level=<?php echo $level ?>";
				window.location.href = url;
  			}
		}
		...
		function edit_proxy(name)
		{
			...
			var url = "proxy_edit.php?proxy=" + name + "&level=<?php echo $level ?>";
			window.location.href = url;
		}
		
		function start_proxy(name)
		{
			window.location.href = "proxy_start.php?proxy=" + name +　"&level=<?php echo $level ?>";
		}
		
		function statistics_proxy(name)
		{
			window.location.href = "proxy_statistics.php?proxy=" + name + "&level=<?php echo $level ?>";
		}
        </script>
    </body>
<?php
