                   ...
                   <!-- block -->
						<div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left"><?php echo $lang_array["stream_equilibrium_server_list_text1"] ?></div>
                            </div>
                            <div class="block-content collapse in">
                                <div class="span12">   
                                   	<div class="table-toolbar">
                                      	<div class="btn-group">
                                         	<a href="stream_equilibrium_server_add.php"><button class="btn btn-success"><?php echo $lang_array["stream_server_list_text6"] ?><i class="icon-plus icon-white"></i></button></a>
                                      	</div>
                                   	</div>
...
<?php
										...
										foreach($namess as $names) 
										{
											...
											echo "<td style='vertical-align:middle; text-align:center;'><a href='#' onclick='reboot_server(\"".$names[0]."\")'>".$lang_array["stream_server_list_text11"].
													...
													"</a>&nbsp;&nbsp;<a href='#' onclick='check_live(\"".$names[1]."\",\"".urlencode($names[2])."\")'>".$lang_array["stream_equilibrium_server_list_text2"].
													"</a></td>";
...
?>                       
		...
		function loadXMLDoc(serverid,cmd)
		{
			...
			xmlhttp.open("GET","stream_equilibrium_server_control.php?serverid="+ serverid + "&control=" + cmd,true);
			xmlhttp.send();
		}
		...
		{
			window.location.href = "stream_equilibrium_channel_live_list.php?serverip=" + serverip + "&tip=" + tip;
		}
		
		function check_live(serverip,tip)
		{
			window.location.href = "stream_equilibrium_channel_live_list.php?serverip=" + serverip + "&tip=" + tip + "&type=live";
		}
		...
		function edit_server(serverid)
		{
			...
			window.location.href = "stream_equilibrium_server_edit.php?serverid=" + serverid;
		}
		
		function run_info(serverid)
		{
			window.location.href = "stream_equilibrium_server_info.php?serverid=" + serverid + "&control=runall";
		}
		...
		function delete_server(serverid)
		{
			if(confirm("<?php echo $lang_array["stream_server_list_text8"] ?>： " + serverid + " ?") == true)
  			{
				window.location.href = "stream_equilibrium_server_del.php?serverid=" + serverid;
			}
		}
        </script>
    </body>
