...
<html>
    ...
    <body>
                   ...
                   <!-- block -->
						<div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left"><?php echo $lang_array["playback_type_list_text1"] ?></div>
                            </div>
                            <div class="block-content collapse in">
                                <div class="span12">                                     
                                	<div class="table-toolbar">
                                      <div class="btn-group">
                                         <a href="playback_type_add.php"><button class="btn btn-success"><?php echo $lang_array["playback_type_list_text2"] ?><i class="icon-plus icon-white"></i></button></a>
                                      </div>
                                      <div class="btn-group">
                                         <a href="#" onClick="edit_password()"><button class="btn btn-success"><?php echo $lang_array["playback_type_list_text3"] ?></button></a>
                                      </div>
                                   	</div>                                  
		...
		<?php
			...
			$type_sql->create_table($mydb, $mytable, "name text, id text ,need int, typepassword text, param0 text, param1 text, param2 text, param3 text, lang text");
			...
			foreach($type_namess as $type_names) {
				...
				if($type_names[2] != null || $type_names[3] != null)
				{
					if($type_names[2] == 0)
					{
					...
					{
						echo "<td style='vertical-align:middle; text-align:center;'>" . $type_names[3] . "</td>";
					}
				}
				else
				{
				...
				echo "<a href='playback_type_edit.php?id=" . $type_names[1] . "'>" . $lang_array["edit_text1"] . "</a>";
				echo "&nbsp;&nbsp;";
		...
		function delete_live_type(name,id)
		{
			if(confirm("<?php echo $lang_array["live_type_list_text5"] ?>： " + name + " ?") == true)
  			{
				var url = "playback_type_del.php?id=" + id;
		...
		{
			var url = "playback_type_direct.php?id=" + id;
			window.location.href = url;
		}
		...
		function edit_password()
		{
			...
			if(ps.length > 0)
			{
				window.location.href = "playback_type_password.php?ps=" + ps;
			}
		}
