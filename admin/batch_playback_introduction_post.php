<?php
	...
	{
		if($type_namess[$ii][2] == 0)
		{
			$type = $type . $type_namess[$ii][1];
	...
	{
		$sql->delete_data_greater($mydb,$mytable,"urlid",10000);
	}
	...
	$namess = $sql->fetch_datas_order_where_isplayback($mydb, $mytable, "urlid", "urlid", 10000);
	$count = count($namess);
	$id = 10001;
	if($count >= 1)
