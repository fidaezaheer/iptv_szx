...
function meuguia_preview($id,$previewurl)
{
	$previews = "";
	
					
	$file=file_get_contents($previewurl);	
	$file = strstr($file,"<li class=\"subheader devicepadding\">");
	$pos = strpos($file,"<li class=\"divider\"></li>");
	$file = substr($file,0,$pos-strlen("<li class=\"subheader devicepadding\">"));
	...
	for($ii=0; $ii<256; $ii++)
	{
		...
		$pos = strpos($file,"</li>");
		$content = substr($file,0,$pos-strlen("<li>"));
		
		$time = strstr($content,"<div class='lileft time'>");
		$pos = strpos($time,"</div>");
		$time = substr($time,strlen("<div class='lileft time'>"),$pos-strlen("<div class='lileft time'>"));
		
		//echo $time;
		
		$value = strstr($content,"<h2>");
		$pos = strpos($value,"</h2>");
		$value = substr($value,strlen("<h2>"),$pos-strlen("<h2>"));
		$value = trim(transfar_html($value));
...
function update_preview($preview_new,$id,$date)
{
	...
	*/
	...
	$previews = meuguia_preview($urlid,$previewurl);
