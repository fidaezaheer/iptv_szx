...
function tvolleh_preview($id,$previewurl,$date)
{
	//echo "previewurl:" . $previewurl;
	
	
	$file=file_get_contents($previewurl . "&date=" . $date);	
	...
	for($ii=0; $ii<24; $ii++)
	{
		...
		$content = substr($file,$tr_pos,$pos-strlen("<tr"));
		//echo $content;
		
		
		
		$time0 = strstr($content,"<td class=\"time\">");
		$pos = strpos($time0,"</td>");
		$time0_value = substr($time0,strlen("<td class=\"time\">"),$pos-strlen("<td class=\"time\">"));
		
		$content = strstr($content,"</td>");
		
		//echo "time0:" . $time0_value;
		//echo "content:" . $content;
		
		$time_array = array();
		
		$time1 = strstr($content,"<td class=\"time\">");
		$pos = strpos($time1,"</td>");
		$time1_value = substr($time1,strlen("<td class=\"time\">"),$pos-strlen("<td class=\"time\">"));
		for($kk=0; $kk<24; $kk++)
		{
			$time1_value = strstr($time1_value,"<p>");
			if($time1_value == false)
				break;
			$pos = strpos($time1_value,"</p>");
			if($pos < 0)
				break;
				
			$p_value = substr($time1_value,strlen("<p>"),$pos-strlen("<p>"));
			
			$time1_value = strstr($time1_value,"</p>");
			
			//echo "time1_value:" . $p_value;
			
			array_push($time_array,trim($p_value));
		}
		
		//print_r($time_array);
		
		$preview = strstr($content,"<td class=\"program\">");
		//echo "preview:" . $preview;
		$pos = strpos($preview,"</td>");
		$preview = substr($preview,strlen("<td class=\"program\">"),$pos-strlen("<td class=\"program\">"));
		//echo "preview:" . $preview;
		...
		{
			$p_len = 0;
			if(strstr($preview,"</strong>") != false)
			{
				$p_len = strlen("</strong>");
				$preview = strstr($preview,"</strong>");
				if($preview == false)
					break;
			}
			else
			{
				$p_len = strlen("<p>");
				$preview = strstr($preview,"<p>");
				if($preview == false)
					break;
			}
			$pos = strpos($preview,"<b>");
			if($pos < 0)
				break;
			$priview_value = substr($preview,$p_len,$pos-$p_len);
			$preview = strstr($preview,"</p>");
			
			$priview_value = iconv("EUC-KR//IGNORE" , "UTF-8", $priview_value);
			//echo "priview_value:" . $priview_value;
			array_push($priview_array,trim($priview_value));
		}
		
		//var_dump($time_array);
		//print_r($priview_array);
		for($kk=0; $kk<count($priview_array); $kk++)
		{
			$previews = $previews . $time0_value . ":" . $time_array[$kk] . "#" . $priview_array[$kk] . "|";
			//echo $previews;
		}
...
function update_preview($preview_new,$id,$date)
{
	...
	*/
	...
	$previews = tvolleh_preview($urlid,$previewurl,$date);
