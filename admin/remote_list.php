...
<?php
	...
	$namess = $sql->fetch_datas_limit_desc($mydb, $mytable, $offset, $size, "remarks"); 
                                ...
                                <div class="span12">
                                   <div class="table-toolbar">
                                    ...
                                    <div class="control-group">
                                       <label class="control-label" for="focusedInput" style="text-shadow:#0CF"><?php echo $lang_array["remote_list_text7"] ?></label>
                                       <div class="controls">
                                       <input style="width:15px;height:28px;" type="radio" name="status" value="0" id="status0" <?php if($status == null || intval($status)==0) echo "checked"?> /><?php echo $lang_array["remote_list_text9"] ?>&nbsp;
                                       <input style="width:15px;height:28px;" type="radio" name="status" value="1" id="status1" <?php if(intval($status)==1) echo "checked"?> /><?php echo $lang_array["remote_list_text8"] ?>&nbsp;
                                       <input name="" class="btn btn-primary" type="button" value="<?php echo $lang_array["sure_text1"] ?>" onclick="save_status()"/>
                                       </div>
                                   	</div>
...
?>                                       
        ...
        <form name="remote_ctr_form" action="remote_all_post.php" method="post">  
		...
		function go_page()
		{
			...
			var url = "remote_list.php?page=" + (pageid-1);
...
<?php
			if($page-1 >= 0)
			{
				echo "var url = 'remote_list.php?page=".($page-1)."';";
...
?>			
		...
		function remote_ctr(mac,cpuid)
		{
			...
			if(url.length > 7)
			{
				window.location.href = "remote_post.php?mac=" + mac + "&cpuid=" + cpuid + "&url=" + encodeURI(url);
		...
		function remote_ctr_del(mac,cpuid)
		{
			...
			window.location.href = "remote_post.php?mac=" + mac + "&cpuid=" + cpuid;
		...
		{
			var v = GetRadioValue("status");
			window.location.href = "remote_status.php?status=" + v;
		}
