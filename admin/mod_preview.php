...
function mod_preview($id,$previewurl)
{
	$file=file_get_contents($previewurl);	
	
	$file = strstr($file,"<div class=\"channel-time-table\">");
	$pos = strpos($file,"<div class=\"logo-card\">");
	$file = substr($file,0,$pos-strlen("<div class=\"channel-time-table\">"));
	
	//echo $file;
	
	for($ii=0; $ii<512; $ii++)
	{
		$file = strstr($file,"<time class=\"time\">");
		$pos = strpos($file,"</time>");
		$time = trim(substr($file,strlen("<time class=\"time\">"),$pos-strlen("<time class=\"time\">")));
		$times = explode(":",$time);
		if(count($times) == 2 && intval($times[0]) > 12 && $ii < 2)
		{
			$file = strstr($file,"<div class=\"wrapper-m\">");
			if($file == false)
				break;
			continue;
		}
		
		$file = strstr($file,"<h4 class=\"heading--regular\">");
		$pos = strpos($file,"</h4>");
		$value = trim(substr($file,strlen("<h4 class=\"heading--regular\">"),$pos-strlen("<h4 class=\"heading--regular\">")));
...
function update_preview($preview_new,$id,$date)
{
	...
	*/
	...
	$previews = mod_preview($urlid,$previewurl);
