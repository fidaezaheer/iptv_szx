...
function ipLocation($datfile = "admin/QQWry.Dat")
{
	include_once 'admindir.php';
	$a = new Adminer();
	$admindir = $a->ad;

  $this->fp=fopen($admindir . "QQWry.Dat" ,'rb')or die("QQWry.Dat不存在，请去网上下载纯真IP数据库, 'QQWry.dat' 放到当前目录下");   //二制方式打开
...
function getIP() {
		...
		elseif (getenv('HTTP_FORWARDED')) {
 		...
 		elseif (getenv('True-Client-IP')) {
                $ip = getenv('True-Client-IP');
        }
		else { 
...
function getaddress($ip){
 ...
 function getcode($ip)
 {
	...
	$gi = geoip_open("admin/GeoIP.dat",GEOIP_STANDARD);
